...
DBConnectionPool::DBConnectionPool()
    : user_name_(sql::SQLString("")), password_(sql::SQLString("")), url_(sql::SQLString("")),
    max_pool_size_(0), current_pool_size_(0), thread_finish_(false), driver_(nullptr)
{}
...
{
    max_pool_size_ = atoi(GConfig.GetConfigParam("db.maxpoolsize", "10").c_str());
    user_name_ = sql::SQLString(GConfig.GetConfigParam("db.uesr", "zhu"));
    password_ = sql::SQLString(GConfig.GetConfigParam("db.password", "no080740"));
    ostringstream os;
    os << "tcp://" << GConfig.GetConfigParam("db.ip", "127.0.0.1") << ":"
        << GConfig.GetConfigParam("db.port", "3306") << "/" << GConfig.GetConfigParam("db.scheme", "ddz");
    url_ = sql::SQLString(os.str());

    try
    {
        // 获取driver
        this->driver_ = sql::mysql::get_mysql_driver_instance();
        logger_info("url:{} 用户名:{} 密码:{}", url_.c_str(), user_name_.c_str(), password_.c_str());
    }
...
void DBConnectionPool::ShrinkConnectNum()
{
    ...
    // 检查间隔
    int check_interval = atoi(GConfig.GetConfigParam("db.checkinterval", "10").c_str());
    // 检查时间
